"""Shared primitives for Environment Discovery V3."""
...
class EnvironmentInputError(ValueError):
    """Raised when an Environment Discovery input cannot be interpreted safely."""
...
def read_json(path: Path) -> Any:
    try:
        ...
        raise EnvironmentInputError(f"cannot read {path}: {error}") from error
    except json.JSONDecodeError as error:
        raise EnvironmentInputError(f"cannot parse JSON {path}: {error}") from error
