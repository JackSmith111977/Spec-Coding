...
首次映射在工具中显式列出11个流程及4规则，登记改变时维护者先解释影响，不自动猜测新来源。后续增量范围仍按治理从上一正式Release的source_revision比较两侧正文和Manifest并传播依赖；本工具不声称实现自动语义增量编译。
...
全部被选试验结束后，`export_harness_evidence.py <目录名...> --output <包外ZIP路径>`保存原始记录与项目Git快照，绑定每份文件Hash。证据包不重复Harness正文，不自动判定PASS；独立判定报告另存verification/harness/。归档过程中发现已枚举文件变更即失败，不用于并发运行中的最终证据冻结。
