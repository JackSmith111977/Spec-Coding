...
**一句接入指令，为当前 Coding Agent 和项目环境建立适配的完整 Harness。**
...
可以是已有代码仓库，也可以是准备开始的新项目目录。Agent 需要能读取发行包和项目文件，并执行接入、适配与验收所需的命令。
...
接入指令无需附带具体开发目标。首次接入时，Agent 会了解你的使用意图、项目现状和可用工具，建立完整流程与规则的可用入口，并验证实际加载与行为。完整 Harness 保留后续流程入口，具体内容在执行时按需读取。需要你决定接入范围或操作权限时，它会说明依据和待决定事项。

接入完成后，再由流程承接具体目标，例如开始新项目、继续已有工作或排查故障。已有绑定与有效证据会被复用，环境变化时重新核验受影响部分。
