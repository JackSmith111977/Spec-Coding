...
项目长期上下文明确区分现状、目标模型与假设。01A 保留其规范要求的目标、业务定义、系统定义及假设，不因存入 project/ 就要求已经实现；01B 及标为 As-Is 的模型以已验证现状为准。需求与设计可以描述目标状态。设计通过不代表代码已实现；项目现状更新要依据实际变更和验证，不能提前把拟议方案写成当前系统行为。取代旧正文时保留版本与替代关系，历史验证仍绑定当时的代码/产物身份；普通导航读取当前正文，复核历史结论读取其对应版本。
...
摘要依据覆盖实际决定该结论的来源及适用范围；仅核对 tasks.md 不能证明相关 OI、共享报告或代码未变化。同一 Commit 下存在未提交修改时，Commit 身份也不足以证明正文同步。不能证明时回读必要来源，不据摘要推进状态或 Gate。
...
维护与恢复要求不授予新的写权限。当前职责只读时，定位错误来源及影响，阻断依赖错误摘要的动作并交由获授权 Owner 修复后重验；仍可继续不依赖该缺口的只读工作，不为修索引擅改被验对象。
