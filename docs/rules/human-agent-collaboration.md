...
Authority 仍复用 [`global-contracts.md`](global-contracts.md) 中 Human / Agent Authority 的规范语义；Decision Readiness 只约束**如何使已需要的 Human 判断具备有效上下文**，不把 Autonomous 动作升级为人工审批。Meta Protocol 引用该 Authority 语义不代表继承 Global Contracts 中仅属于 Workflow 的 Task / Gate 等执行契约。
