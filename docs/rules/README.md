...
本目录维护跨阶段持续适用的正式 Rule（规则）。Rule 定义执行过程中需要持续遵守的原则、约束或质量要求，本身不推进 Workflow（流程）状态，也不形成新的阶段。
...
正式 Rule Document 以 [`../manifest.yaml`](../manifest.yaml) 的 `rule_documents` 为机器可读入口。Workflow 文档只引用适用规则，不复制规则正文形成并行事实源。
