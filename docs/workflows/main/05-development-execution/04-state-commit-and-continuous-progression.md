...
Worker 日志、中间推理、重复调试过程不作为长期事实保存。遵循[产物组织与读取规则](../../../rules/artifact-organization-and-reading.md)：必要证据固定后写回权威产物，再同步直接索引并检查引用。并发遵循单写入者边界；中断恢复核对原状态而非旧摘要，被正式结论引用的临时证据在清理前迁入稳定位置或保留。
...
只有上游纠偏使下游 Task Contract 本身失效时，才调整下游状态并重新规划 / 有效性确认。
...
- **Requirement Sync（需求同步）**：仅聚合 Requirement Integration、Requirement AC Gate、Requirement Push 的状态及必要 `code_ref` 供下游消费；三者仍是底层事实，Requirement Sync 不形成第四个权威状态事实源。
- Requirement Push **不等于** Merge（合并）、Release（发布）、Deploy（部署）或 Stage 6 的最终 `Verified（已验证）`。
- Integration、AC Gate 或 Push 任一步失败时，保留已确认 Task 事实，更新 Requirement Sync 聚合视图并路由到最短修复 / 纠偏路径；只有 Task Contract 因上游修正失效时才回退其状态。
...
- Requirement Sync 显示需求级同步尚未完成 → 继续处理对应 Integration、AC Gate 或 Push 问题。
...
| `code_ref` | 已正式验证的 Task Commit 或其他代码引用；无则省略。 |
...
> **将执行结果固化为可信任务事实，以 Task Commit 形成任务级代码边界，以 Requirement Integration + AC Gate + Push 形成需求级 Git 边界，并通过非权威的 Requirement Sync 聚合视图支持下游消费，持续驱动执行直到开发实施整体收敛。**
