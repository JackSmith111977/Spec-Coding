...
基于真实执行证据，判断 Spec Coding / SDD 的**可复用 Workflow、Rules 与 Harness 是否需要改进**。
...
`EV-xxx`、`ISS-xxx`、`RC-xxx`、`IMP-xxx`，以及经验证演进后的 Workflow / Rules / Harness。

## 完成条件

根因已经转化为明确改进或确认 `No Process Change`；需要实施的改进已进入验证闭环，并能在后续真实流程中判断其效果。

> 本阶段只改进可复用 SDD / Harness 机制本身，包括正式 Workflow、Rules 与 Harness，不修改某个具体项目的 Requirement、Design、Task 或代码产物。
