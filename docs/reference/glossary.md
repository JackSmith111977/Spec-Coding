...
- **Artifact Entry（产物入口）**：定位相关对象、权威正文及证据的导航；不自动拥有业务状态。
- **Authoritative Content（权威正文）**：由原 Workflow 或 Meta Protocol 指定、承担事实与状态的正式内容，可以是文件、章节或既有外部记录。
- **Derived Navigation（派生导航）**：引用原事实源的目录/索引/摘要，不形成第二套可独立写入状态。
- **Workspace Mapping（工作空间映射）**：Spec Workspace 中逻辑产物角色到既有位置的稳定绑定，不保存动态 Runtime 事实。
