...
2. 读取[全局权限段](../rules/global.md)及[人机协作](../rules/collaboration.md)、[产物组织与读取](../rules/artifacts.md)，复用已有意图和授权；一句指令不扩大权限。涉及委派、隔离、独立审查或能力路由前读取[委派规则](../rules/delegation.md)，适用于接入动作。仅接入无需业务目标，不生成虚构业务产物。
...
5. 发现实际Runtime/项目环境，装配到真实加载面，固定本地候选，对照原包回查语义并验收加载、产物导航/写回与行为。环境及工具检查先行，需要独立推理时将稳定候选批量送审，修复后按影响由原独立Reviewer复核，必要首次加载/新会话恢复仍用Fresh上下文；按[能力要求](requirements.md)核对证据复用条件，不转移旧候选整体通过结论。READY仅对已验证Runtime、项目和范围成立，文件存在或安装成功不等于已加载。
6. 正式接入通过后，有明确启动新业务工作或继续既有工作的意图则进入或恢复适当Workflow，不要求已有Task；明确仅接入则汇报结果并结束。候选测试只运行授权场景，到停止条件返回。包语义缺失返回维护者，不读浮动源码补缺或重新预编译。
