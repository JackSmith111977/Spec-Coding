...
| artifact-navigation：全部相关产物动作 | 稳定空间与作用域、权威正文/索引分离、统一任务集、原OI、必需阻塞可见、先正文后导航、原包与适配身份分离 | 基础Markdown/既有文档或状态系统；Fresh从真实入口定位当前契约及跨需求依赖，陈旧索引定向回查，写回/迁移中断恢复不丢状态与证据，不全文读无关历史 | 布局绑定错回Onboarding，加载错回适配，缺证/状态错回原Owner；只阻断依赖缺口的动作 |
