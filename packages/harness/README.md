...
正式发布时，验证报告、兼容实测和发布元数据由可信发行入口在冻结包外提供；本候选不声明已有Release或发布通过证据。受控测试授权及记录始终在包外，不进入冻结包。发行入口无可信通过依据则正式接入BLOCKED。客户端执行语义全部在包内，报告不补缺失行为。
...
本版本按[委派规则](rules/delegation.md)优先由Main与工具完成确定性检查，稳定候选批量独立审查，修复定向复核，必要Fresh与证据复用边界保持有效；[产物组织与读取](rules/artifacts.md)继续提供空间入口、权威正文与按需导航，并支持仅接入结束。已有绑定不默认升级；相关环境变化重验受影响机制，候选测试不代替真实项目验收。历史版本用Git/Release，不在包内维护副本。
