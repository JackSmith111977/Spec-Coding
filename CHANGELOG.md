...
- 按用户明确指定的Patch方式构建完整增量候选；本轮新增验证成本治理及维护工具能力，依原规则通常属于MINOR，本次修订号为明确的版本选择例外，不宣称全部变更语义不变。
- 委派与验证流程接通稳定候选批量审查、原Reviewer定向复核、必要Fresh隔离、按影响重验和有依据的证据复用；仍保留独立性、代码对象和正式Gate要求。
- 增加Patch范围计算、基线Hash核验、范围外/入口契约变化拒绝、独立输出目录冻结及完整可复现ZIP构建工具。
- 工具验证21项通过（20项回归及1项真实Git/CLI集成），不等于候选包的独立语义、行为验证或正式发行通过。
- 固定完整增量包通过全量结构、独立增量语义及必要接入／新会话恢复集成验证；范围和首次覆盖阻断的补证过程见[发行判定](verification/harness-reliability/release-0.13.1/stage-3.md)。旧0.13.0发行包保持不变，不宣称全Runtime或实际成本下降。

## 0.13.0 - 2026-09-09

- 新增产物组织与读取规则，统一工作空间入口、权威正文、按需索引、共享产物、证据及临时材料边界；已有空间优先复用。
- 两个接入协议接通目录映射、真实加载与适配记录，支持迁移恢复及仅建立 Harness 后结束；任务状态与开放项保持原事实源。
- 从45份Canonical构建11个Skills、5类共享规则及完整接入入口；修正开发流程Task Graph Update必需字段被弱化的问题。原样发布固定R2，11项工具测试通过。
- 独立语义与代表性行为验证完成；保留R1语义阻断、R2首次状态越界、CLI隔离失败与写入限制，以及后续修正和真实本机同步证据。范围见[最终判定](verification/artifact-governance/stage-3.md)，不宣称跨平台、效率对照或真人审阅已完成。
- 优化根README接入指令、产物位置与升级说明；维护规则明确Release标题直接使用Tag。

## 0.12.0 - 2026-09-07

- 从44份完整Canonical首次构建预编译Harness，提供11个Skills、4类共享规则及Bootstrap、路由、能力要求和完整性工具；结构、独立语义与必要行为验证已通过。
- 增加包完整性、来源映射和可复现归档工具；10项工具测试通过，20项行为挑战完成。发布原样R2包及包外验证证据，目标项目仍需自行完成接入与适配验收。
- CLI跨会话状态收口遗漏经独立评审发现、Owner追加纠正并独立复核；保留首次BLOCKED及前后证据归档，不宣称零提醒一次全过。兼容范围限本轮实测Windows/Codex配置。

- 新增 Harness Adoption & Adaptation 元协议，定义包消费契约及目标侧读取、环境发现、适配、验收和恢复流程。
- Harness Build & Release 按同一契约提供安装前入口、预编译接入程序、适配要求与验收依据，并补充消费者交接验证。
- 增量构建纳入 Manifest 登记、适用范围和路由变化，按旧、新关系及依赖扩展消费者和验证范围。
- 本地适配增加对原始包的语义回查，固定验收预期；将旧 Harness Compilation 职责迁移到维护者预编译、目标侧适配和运行时调度。
- 定义维护者受控候选验证入口，允许固定未发布候选在授权测试范围内完成接入链路，测试证据不替代正式发行与项目验收。
- Manifest登记第2份Meta Protocol，同步接入导航与术语；当前正式版本为`0.12.0`，发行身份与包外通过依据见`verification/harness/publication.md`。
...
- 新增工具级确定性、失败路径、语义审查场景和受控目标级 E2E 测试；新增针对当前 Canonical Corpus 的自举编译测试夹具、来源新鲜度检查与无效条件 Probe。
...
- 已编译的自举 Harness 从根目录移至 `tests/harness_compiler/fixtures/spec-coding-harness/`；根目录不再包含 `AGENTS.md` 或 `harness/` 运行时入口，测试夹具明确不作为项目指令加载。

### Notes｜说明

- `VERSION` 与 Manifest 版本已同步为 `0.11.0`；Git Tag 仍应在功能分支通过完整验证并合并后创建。
