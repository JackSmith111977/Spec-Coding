...
- 从完整 Canonical 首次构建预编译 Harness，目标包含主流程、异常流程、接入程序和共享规则；发布以固定候选的结构、独立语义与行为验证全部通过为条件。
- 增加包完整性、来源映射和可复现归档工具；验证证据保存在包外，候选试验不代表正式发行。
...
- Manifest 登记第 2 份 Meta Protocol，同步接入导航与术语；当前版本身份为 `0.12.0 candidate`，验证与发布状态以包外证据为准。
