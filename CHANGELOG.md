...
- 规划 `0.13.0`：[产物组织与渐进读取设计](docs/governance/designs/artifact-organization/README.md)，覆盖统一工作空间布局、权威产物与索引分离、按需读取、适配记录、迁移恢复及验收。设计已进入实现，当前候选尚未发布。
- 明确GitHub Release标题直接使用对应Git Tag名称，版本内容与兼容说明写入正文，并在发布后核对标题一致性。

## 0.13.0 - 候选验证中

- 新增产物组织与读取规则，统一工作空间入口、权威正文、按需索引、共享产物、证据及临时材料边界。
- 两个接入协议建立和验收目录映射，支持迁移恢复、Fresh 读取及仅接入请求；任务状态与开放项保持原事实源。
- 当前正在构建与验证新版 Harness，正式发行状态以实际发布记录为准。
